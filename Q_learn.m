function [Q,Q2,t_c,t_v,targ_pols,virt_pol_log,targ_pol_ret]=Q_learn(Q,Q2,t_c,t_v,targ_pols,targ_pol_ret,p)
state=p.start;
targ_found=0;
traj=[];
pol=[];
...
while norm(state(1:2)-p.target)>p.target_thresh&&cnt<2000%till goal is reached or max agengt-env interactions are reached
    ...
    err=reward2+p.gamma*(Qmax_next)-Q2(state(1),state(2),a);%TD error for secondary task
    Q2(state(1),state(2),a)=Q2(state(1),state(2),a)+p.alpha*(err);%Q learning update

    rew_hist=[rew_hist;reward2];%save reward history
          ...
          if length(virt_targ_pols{ii})>0
              for kk=1:(s_p_h(1)-1)
                  t_v=t_v+1;%count number of replayed experiences
                   [Qmax_v,a_next]=maxQ(Q2,[pol_hist(kk,1),pol_hist(kk,2) 0]);
                  v_err=pol_hist(kk+1,end)+p.gamma*Qmax_v-Q2(pol_hist(kk+1,1),pol_hist(kk+1,2),pol_hist(kk+1,4));%TD error
                  Q2(pol_hist(kk+1,1),pol_hist(kk+1,2),pol_hist(kk+1,4))=Q2(pol_hist(kk+1,1),pol_hist(kk+1,2),pol_hist(kk+1,4))+p.alpha*v_err;%Q learning update
              end             
          end
      end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
