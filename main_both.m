...
% for i=1:p.N_iter
%     i
%     [Q,Q2,t_c,t_v,targ_pols,virt_pol_log,targ_pol_ret]=Q_learn(Q,Q2,t_c,t_v,targ_pols,targ_pol_ret,p);
%     virt_pol_log_all{i}=virt_pol_log;
...
% % % % Qmap=gen_Q_map(Q);
% 
% [convergence]=evaluateQ(Q2,p)
% conv_log(kkk)=convergence;
% tvlog(kkk)=t_v;
% tclog(kkk)=t_c;
% end
% % save Q21e6 Q
...
for i=1:p.N_iter
    ...
    if length(targ_pols)>0
        targ_pol_ret=-10000*ones(length(targ_pols),1);
    end
    [Q,Q2,t_c,t_v,targ_pols,virt_pol_log,targ_pol_ret]=Q_learn_both(Q,Q2,t_c,t_v,targ_pols,targ_pol_ret,p);
    virt_pol_log_all{i}=virt_pol_log;
